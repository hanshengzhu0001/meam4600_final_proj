...
\label{fig:workflow}
\end{figure*}
Figure~\ref{fig:workflow} provides the end-to-end study logic used in this report, from PDE-family data preparation to objective-wise projection-policy ranking.
...
The winners are numerically distinct rather than tied, which confirms that projection policy should be selected per objective. A single global policy is suboptimal when deployment priorities differ, consistent with the quality-runtime frontier in Figure~\ref{fig:tradeoff}.
...
Figure~\ref{fig:gaincost} summarizes the same result as a metric-by-metric gain-cost scorecard.
...
\label{fig:guidance}
\end{figure}
The guidance sweep trend in Figure~\ref{fig:guidance} supports the choice of stronger guidance for inverse reconstruction quality.
...
\label{fig:tradeoff}
\end{figure*}
Figure~\ref{fig:tradeoff} shows that quality-optimal, physics-optimal, and runtime-optimal points are separated in policy space.
...
\textbf{Interpretation.} Stronger guidance helps every tested family, with gains ranging from \(-9.03\%\) to \(-29.84\%\), supporting the robustness of guidance tuning beyond one PDE type; Figure~\ref{fig:crossfamily} visualizes these cross-family gains.
...
The study intentionally prioritizes interpretability and reproducibility over exhaustive benchmarking. Full cross-method reproduction of all published baselines is outside scope. Nonetheless, all experiments are script-driven, metric definitions are explicit, and artifacts are tracked in the repository.
